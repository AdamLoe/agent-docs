...
record is a single IO step, so it runs inline — do not invent a worker for a
one-line append.
...
step. Spawn a worker only when judgment genuinely needs isolation:

- **Payload validation or append path needs isolated judgment** → profile
  `maintenance.docs`.
