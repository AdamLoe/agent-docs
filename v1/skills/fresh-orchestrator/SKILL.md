...
3. `${CLAUDE_PLUGIN_ROOT}/rules/orchestrating.md` — the generic orchestration discipline (delegation, sequencing, the standard sub-agent preamble, when to involve the lead). This is your operating manual; follow it.
...
Do not read architecture, decisions, or other agent-context docs proactively. Route to the smallest matching subtree only when a specific stream needs it (see `docs/index.md` for the global router), and push that reading into sub-agents rather than your own context. Code paths in docs are relative to `src/`.
