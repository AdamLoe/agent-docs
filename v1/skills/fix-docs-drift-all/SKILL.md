...
> literal counts (see authoring rules 3 and 3a), so the sweep does **not**
> mirror a count list. Instead it verifies that the docs' `path → symbol`
> pointers still resolve, flags any forbidden transcription that crept back,
> and spot-checks high-risk facts against the code that owns them. Code is
> authoritative; the sweep moves the doc toward the code, never the reverse.
...
needs human judgment. It must still be frugal — bounded agents, the cheap
model by default, the expensive one only where the reasoning is genuinely
hard.
...
The **codebase is the source of truth for behaviour**; **docs are the
source of truth for what's-where and why.** So:
...
- **Escalate, don't guess** (report for the user, change nothing):
  missing decision *rationale*, ownership questions, anything needing a
  design judgment, or a mismatch where the **doc may be the intended spec
  and the code is the bug**. Do **not** rewrite a doc to bless behaviour
  that looks like a bug — flag it.
...
- **Tier each cluster:**
  - **Opus** — clusters that touch a high-risk contract surface (from the
    `drift-verification` slot of `docs/_meta/manifest.md`) or that require
    multi-file reconciliation. Cross-file contract reasoning is where the
    cheap model slips.
  - **Sonnet** — everything else (mechanical verify-and-fix).
- Aim for **~5–8 agents total, run in parallel**. Do **not** spawn one
  agent per doc — batch by cluster. Keep Opus to the few contract clusters.
...
For each cluster, spawn a `general-purpose` agent (it needs Edit/Write —
the read-only Explore agent can't fix), with `model: sonnet` or `model: opus`
per the tier. Run them in parallel. Give each agent:
